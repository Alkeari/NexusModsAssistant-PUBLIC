...
async function run() {
    ...
    await check('tests', async () => {
        const result = await nma.runNode(['--test', 'tests/*.test.js'], { capture: true, timeoutMs: 300000 });
        if (result.code !== 0) {
            throw new Error('node --test failed:\n' + (result.stdout + result.stderr).split(/\r?\n/).slice(-25).join('\n'));
        }
        const pass = (result.stdout.match(/^\W*pass (\d+)\s*$/m) || [])[1];
        const skip = (result.stdout.match(/^\W*skipped (\d+)\s*$/m) || [])[1];
        return (pass || '0') + ' passed, ' + (skip || '0') + ' skipped';
    });
    ...
    if (phase === 'post-build') {
        ...
        if (targets.includes('firefox')) {
            ...
            await check('firefox.source-archive', () => {
                ...
                const required = ['BUILD-INSTRUCTIONS.md', 'package.json', 'package-lock.json', 'webpack.config.js', 'tsconfig.json'];
